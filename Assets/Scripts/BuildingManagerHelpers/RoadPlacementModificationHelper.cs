...
public class RoadPlacementModificationHelper : StructureModificationHelper
{
	...
	{
		var neighbourStatus = RoadManager.GetRoadNeighbourStatus(gridPosition, grid, structuresToBeModified);
		...
		roadToReturn = RoadManager.CheckIf3WayFits(neighbourStatus, roadToReturn, structureData);
		if (roadToReturn != null)
		{
			return roadToReturn;
		}
		roadToReturn = RoadManager.CheckIf4WayFits(neighbourStatus, roadToReturn, structureData);
		if (roadToReturn != null)
		{
			return roadToReturn;
		}
	...
	public void ModifyRoadCellsOnTheGrid(Dictionary<Vector3Int, GameObject> neighboursDictionary, StructureBaseSO structureData)
	{
		foreach (var keyValuePair in neighboursDictionary)
		{
