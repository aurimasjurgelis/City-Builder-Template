...
public static class StructureEconomyManager 
{
	...
	public static void PrepareZoneStructure(Vector3Int gridPosition, GridStructure grid)
	{
		...
		{
			var structuresAroundPositions = grid.GetStructuresPositionsInRange(gridPosition, zoneData.maxFacilitySearchRange);
			foreach(var structurePositionNearby in structuresAroundPositions)
			{
				...
				if(data.GetType() == typeof(SingleFacilitySO))
				{
					...
					{
						if(grid.ArePositionInRange(gridPosition, structurePositionNearby, facility.singleStructureRange))
						{
	...
	{
		return (zoneData.requirePower) && zoneData.HasPower() == false || zoneData.requirePower && zoneData.HasWater() == false;
	}
	...
	public static IEnumerable<StructureBaseSO> PrepareRoadDemolition(Vector3Int gridPosition, GridStructure grid)
	{
		...
		return roadData.PrepareRoadDemolition(structureAroundRoad);
	}
